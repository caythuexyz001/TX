...
from app.api.schemas import IngestIn, PredictOut, StatsOut, HistoryOut, SummaryOut, PredictionItem
from app.services import ingest_round, get_stats, get_patterns, predict_by_markov, get_history, get_summary
...
r, resolved = ingest_round(session, data.d1, data.d2, data.d3, data.md5)
resp = {
'stored_round': {
...
if resolved:
resp['resolved_prediction'] = {
'id': resolved.id,
'md5': resolved.md5,
'label_pred': resolved.label_pred,
...
async def history(limit: int = 50, session: Session = Depends(get_session)):
items = get_history(session, limit=limit)
return {'items': items}
