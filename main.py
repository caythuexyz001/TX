...
async def sec_limit(request: Request, call_next):
    headers={"X-Frame-Options":"DENY","X-Content-Type-Options":"nosniff","Referrer-Policy":"same-origin","Cache-Control":"no-store"}
...
# --- Quản lý session + trạng thái FIBO ---
class SessionItem:
    def __init__(self):
        self.ai = TaiXiuAI()
        self.last = time.time()

        # Bankroll (VND) + Fibonacci staking
        self.bank_vnd: int = 0               # số vốn hiện có
        self.fib_base: int = 0               # đơn vị cơ sở (VND)
        self.fib_max: int = 12               # giới hạn số bước
        self.fib_idx: int = 0                # vị trí hiện tại trong dãy
        self.fib_seq: List[int] = [1,1]      # dãy hệ số (nhân với base)
        self.last_bet_vnd: int = 0           # gợi ý cược lần trước

    def ensure_seq(self):
        # tạo dãy fib đủ dài (an toàn tối đa self.fib_max)
        self.fib_seq = [1, 1]
        while len(self.fib_seq) < max(2, self.fib_max):
            self.fib_seq.append(self.fib_seq[-1] + self.fib_seq[-2])

    def next_fib_bet(self) -> int:
        """Tính số tiền cược VND theo Fibonacci, có kẹp an toàn."""
        ...
        # Kẹp an toàn: mỗi ván không quá 20% vốn còn lại; đồng thời chẵn 1,000đ
        cap = int(max(0, self.bank_vnd) * 0.2)
        bet = min(raw, cap if cap>0 else raw)
        bet = (bet // 1000) * 1000  # làm tròn xuống 1,000đ
        return max(0, bet)

    def apply_round_result(self, win: bool):
        """Cập nhật FIBO sau 1 ván (giả định kèo 1:1)."""
        bet = self.last_bet_vnd
        if bet > 0:
            # PnL 1:1
            if win:
                self.bank_vnd += bet
                # Quy tắc Fibo: thắng lùi 2 bậc, thua tiến 1 bậc
                self.fib_idx = max(0, self.fib_idx - 2)
            else:
                self.bank_vnd -= bet
                self.fib_idx = min(self.fib_idx + 1, self.fib_max - 1)

        # Nếu vốn tụt xuống 0 → reset nhẹ
        if self.bank_vnd <= 0:
            self.bank_vnd = 0
            self.fib_idx = 0
...
def get_ai(request: Request, response: Optional[Response]=None)->TaiXiuAI:
    _gc_sessions()
    sid=request.cookies.get("txsid")
    if not sid or sid not in SESSIONS:
        sid=uuid.uuid4().hex; SESSIONS[sid]=SessionItem()
...
def outcome_from_dice(d: List[int])->str:
    return "Triple" if d[0]==d[1]==d[2] else ("Xỉu" if sum(d)<=10 else "Tài")
...
def set_algo(body: AlgoSelectBody, request: Request):
    ai=get_ai(request)
    for i,(n,_) in enumerate(ai.algorithms):
        if n==body.name: ai.current_algo_index=i; return {"ok":True,"current_algo":n}
...
def set_combo(body: ComboSelectBody, request: Request):
    ai=get_ai(request)
    for i,(n,_) in enumerate(ai.combo_strategies):
        if n==body.name: ai.current_combo_idx=i; return {"ok":True,"current_combo_algo":n}
    raise HTTPException(status_code=400, detail="Combo strategy not found")
...
def step1_predict(body: Step1Body, request: Request):
    ...
    # cập nhật bankroll/Fibo nếu có input
    ...
    # nếu có ván trước → update
    if body.prev_dice:
        ai.update_with_real(body.prev_md5 or "", outcome_from_dice(body.prev_dice), body.prev_dice)

    # tạo dự đoán ván kế
    out = ai.next_prediction(md5_value=body.next_md5 or "", alpha=body.alpha,
                             window_n=body.window_n, temperature=body.temperature)

    # Gợi ý cược theo Fibonacci
    ...
    return {
        ...
        "fib": {
            ...
            "seq_preview": get_seq_preview(sess)
        },
        "echo": {"promote_next_md5_to_prev": body.next_md5 or ""}
    }

def get_seq_preview(sess: SessionItem):
    sess.ensure_seq()
    # preview 8 bước kể từ idx hiện tại
...
def step2_confirm(body: Step2Body, request: Request):
    ai=get_ai(request); sess=get_sess(request)
    if body.dice:
        real=outcome_from_dice(body.dice); dice=body.dice
    elif body.real:
        real=body.real; dice=[0,0,0]
    else:
        return {"error":"Cần real hoặc dice"}

    rec=ai.update_with_real(body.md5 or "", real, dice)
    # cập nhật FIBO theo kết quả
    ...
    return {"record": rec,
            ...
            "fib": {
                "bankroll_vnd": sess.bank_vnd,
                "idx": sess.fib_idx,
                "next_bet_vnd": sess.next_fib_bet(),
                "seq_preview": get_seq_preview(sess)
            }}
...
<style>
:root{--bg:#13070a;--bg2:#2a0f13;--card:#1b0c0f;--txt:#fff3cc;--muted:#ffddb0;--gold:#ffd54a}
*{box-sizing:border-box}body{margin:0;color:var(--txt);
background:radial-gradient(1200px 700px at 70% -10%,#3a0f14 0%,var(--bg2) 30%,var(--bg) 70%) fixed;
...
.card{background:linear-gradient(180deg,#1f0e12,var(--card));border:1px solid rgba(255,213,74,.35);border-radius:16px;padding:18px;box-shadow:0 12px 40px rgba(225,29,42,.25), inset 0 0 0 1px rgba(255,255,255,.02);margin-bottom:16px}
...
.row{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.row2{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
.actions{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
.btn{padding:12px 16px;border:0;border-radius:12px;cursor:pointer;font-weight:700}
.btn-primary{background:linear-gradient(90deg,#e11d2a,#ff3838,#f5d200);color:#2d0b0f}
...
.hidden{display:none}
.badge{background:linear-gradient(90deg,#f5d200,#ffd54a);-webkit-background-clip:text;background-clip:text;color:transparent;font-weight:900}
.vnd{font-variant-numeric:tabular-nums}
</style></head><body>
<div class="wrap">
 <div class="header">
   <div style="font-weight:900;font-size:22px">TaiXiu AI <span class="badge">#tool make by Boring</span></div>
   <div style="display:flex;gap:8px">
     <button class="btn btn-ghost" onclick="copyLink()">Copy link</button>
     <button class="btn btn-ghost" onclick="downloadCSV()">Tải thuật toán CSV</button>
     <button class="btn btn-ghost" onclick="downloadComboCSV()">Tải combo CSV</button>
     <button class="btn btn-ghost" onclick="loadSummary()">Làm mới</button>
     <button class="btn btn-danger" onclick="doReset()">Reset (admin)</button>
   </div>
 </div>
 ...
 <div id="step1" class="card">
   ...
   <div class="row2" style="margin-top:6px">
     <div><label>Thuật toán (auto)</label><select id="algo-select"></select></div>
     <div><label>Combo (auto)</label><select id="combo-select"></select></div>
   </div>
...
<div id="toast" style="position:fixed;right:14px;bottom:14px;background:#111827;color:#e5e7eb;border:1px solid #374151;padding:10px 12px;border-radius:10px;opacity:0;transform:translateY(8px);transition:all .3s"></div>
<script>
function g(id){return document.getElementById(id)}
function valInt(id){const v=g(id).value.trim();return v===''?null:parseInt(v,10)}
function valNum(id){const v=g(id).value.trim();return v===''?null:parseFloat(v)}
...
async function loadAlgoList(){
  const r=await fetch('/algo_list');const j=await r.json();const sel=g('algo-select');sel.innerHTML='';
  j.algorithms.forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;sel.appendChild(o);});
  sel.value='algo_mix'; await fetch('/set_algo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'algo_mix'})});
  sel.addEventListener('change', async()=>{await fetch('/set_algo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:sel.value})}); toast('Thuật toán: '+sel.value)})
}
async function loadComboList(){
  const r=await fetch('/combo_list');const j=await r.json();const sel=g('combo-select');sel.innerHTML='';
  j.combo_algorithms.forEach(n=>{const o=document.createElement('option');o.value=n;o.textContent=n;sel.appendChild(o);});
  sel.value='combo_mix'; await fetch('/set_combo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'combo_mix'})});
  sel.addEventListener('change', async()=>{await fetch('/set_combo',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:sel.value})}); toast('Combo: '+sel.value)})
}
addEventListener('load',()=>{loadAlgoList();loadComboList();});
...
function render(whereId, roundData, nextData){
  ...
  const fib = nextData.fib||{};
  const seq = (fib.seq_preview||[]).map(x=>formatVND(x)).join(' → ') || '—';
...
const uiState={alpha:1,windowN:null,temp:0.85}; let s2Busy=false;
...
async function doStep1(){
  ...
  uiState.alpha=parseInt(g('s1-alpha').value||'1',10); uiState.windowN=valInt('s1-window'); uiState.temp=parseFloat(g('s1-temp').value||'0.85');

  const body={
    prev_dice: prevDice,
    prev_md5: valStr('s1-prev-md5'),
    next_md5: valStr('s1-next-md5'),
  ...
  let roundData={record:data.summary.last_record, summary:data.summary}; if(!roundData.record){ roundData={record:{}, summary:data.summary}; }
  render('s2-out', roundData, data); swap('step1','step2'); unlockStep2(); toast('Đã tạo dự đoán, sang B2 để xác nhận');
}

async function confirmResult(label){
  if(s2Busy) return; s2Busy=true;
  const md5=valStr('s2-md5');
  ...
  const recData=await recRes.json(); lastRoundForB1.prevMd5=md5||recData.record?.md5||null;

  const pred=await fetch('/step1_predict',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
    alpha:uiState.alpha, window_n:uiState.windowN, temperature:uiState.temp
  })});
  const next=await pred.json(); render('s2-out', recData, next); lockStep2(); toast('Đã cập nhật & tạo dự đoán mới'); s2Busy=false;
}
async function confirmDice(){
  if(s2Busy) return; s2Busy=true;
  const d1=valInt('s2-d1'),d2=valInt('s2-d2'),d3=valInt('s2-d3'); if(!(d1&&d2&&d3)){toast('Nhập đủ 3 xúc xắc'); s2Busy=false; return;}
  const md5=valStr('s2-md5'); const recRes=await fetch('/step2_confirm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({md5, dice:[d1,d2,d3]})});
  const recData=await recRes.json(); lastRoundForB1.prevMd5=md5||recData.record?.md5||null;

  const pred=await fetch('/step1_predict',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
    alpha:uiState.alpha, window_n:uiState.windowN, temperature:uiState.temp
  })});
  const next=await pred.json(); ['s2-d1','s2-d2','s2-d3'].forEach(id=>g(id).value=''); render('s2-out', recData, next); lockStep2(); toast('Đã cập nhật & tạo dự đoán mới'); s2Busy=false;
}
function backToStep1(){ if(lastRoundForB1.prevMd5) g('s1-prev-md5').value=lastRoundForB1.prevMd5; swap('step2','step1'); g('s1-next-md5').focus(); }
...
function doReset(){const k=prompt('Nhập ADMIN API key để reset:'); if(!k) return; fetch('/reset',{method:'POST',headers:{'X-API-Key':k}}).then(r=>toast(r.ok?'Đã reset':'Sai key'))}
</script>
</body></html>
"""

@app.get("/")
def root(): return {"ok":True, "msg":"Open /ui for interface"}

@app.get("/ui", response_class=HTMLResponse)
def ui(request: Request):
    resp = HTMLResponse(content=PAGE); _ = get_ai(request, resp)  # tạo session + cookie
    return resp
